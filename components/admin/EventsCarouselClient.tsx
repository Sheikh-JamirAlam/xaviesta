...
export default function EventsCarouselClient({ slides }: Props) {
  ...
  async function save() {
    ...
    const res = await fetch("/api/admin/business-images", {
      method: "POST",
      body: formData,
      cache: "no-store",
    });
          ...
          {slides.map((src) => (
            <div className="min-w-0 flex-[0_0_100%]" key={src}>
              <div className="h-110 border flex items-center justify-center">
                <Image src={src} alt="Business event" width={1920} height={1080} className="object-cover" unoptimized />
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="absolute z-10 top-4 right-4">
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          ...
          <DialogContent className="sm:max-w-106.25">
            <DialogHeader>
              <DialogTitle>Edit business events images</DialogTitle>
              <DialogDescription className="sr-only">Edit business events images</DialogDescription>
            </DialogHeader>
